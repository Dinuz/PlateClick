...
class ServerHandler():
    ...
    def phase_i(self, uid, choice):
        ...
        try:
            ...
            #if uid not in self.user_dict.keys():
                #Iteration = 15
                #self.user_dict[uid] = YadaUser(self.Dist_matrix_size, Iteration)
            ...
            if self.user_dict[uid].user_iteration <= 2:
                ...
                return []
            #logger.info ('phase I for %d' % uid)
        except KeyboardInterrupt:
            # clean here, e.g. release locks
            pass
    ...
    def _phase_ii(self, uid, choice):
        try:
            ...
            self.user_dict[uid].user_iteration += 1
            return url_list
            #logger.info ('phase II for %d' % uid)
        except KeyboardInterrupt:
            # clean here, e.g. release locks
            pass
    ...
    def request_ranking(self, uid, choice):
        #logger.info(uid)
        #logger.info(choice)
        #logger.info(self.user_dict[uid].test_image_selection)
        #For Demo Purpose: Send back 12 highest preference food items.
        try:
            ...
            # back_images, random_images, positive_images: contains indexes to the original image set
            # i.e. back_images = positive_images + random_images
...
class YadaUser():
    ...
    # def store_metadata(self, uid):

    #     accuracy_list = list()
    #     if self.debug:
    #         logger.info(self.test_image_selection)

    #     testing_list = list()
    #     correct_count = 0
    #     for test in zip(self.test_image_selection, self.test_image_positive, self.test_image_list):
    #         testing_list.append((test[0], test[1], test[2]))
    #         if self.debug:
    #             logger.info(test[0])
    #             logger.info(test[1])
    #             logger.info(test[2])
    #         for selection in test[0]:
    #             if test[2][selection] in test[1]:
    #                 correct_count += 1

    #     accuracy_list.append(float(correct_count) / 10)
    #     user_metadata = dict()
    #     user_metadata["uid"] = uid
    #     user_metadata["accuracy"] = accuracy_list
    #     user_metadata["testing"] = testing_list
    #     user_metadata["preference"] = self.user_preference.tolist()

    #     print "========================================"
    #     print user_metadata["accuracy"]
    #     print "========================================"

    #     if self.debug:
    #         user_metadata["evolution"] = self.preference_evolution
    #         user_metadata["added_vector"] = self.added_vector
    #     with open(self.file_prefix + uid, "w") as f_handle:
    #         json.dump(user_metadata, f_handle)
